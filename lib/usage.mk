...
makephile_usage:
	...
	@echo ""
	@echo "Targets:"
	@$(phil_usage_parse_targets)

##
# Parse the Makefile for targets and their descriptions.
#
define phil_usage_parse_targets
$(phil_grep_multiline) '$(phil_usage_match_target)[a-zA-Z_-]+:.*(\n|$$)' $(MAKEFILE_LIST) \
| grep --extended-regexp --text "^[a-zA-Z_-]+:.*" \
| sort \
| while read -r line; do \
    comment="$$($(phil_grep_multiline) '$(phil_usage_match_target)'"$${line}" $(MAKEFILE_LIST) | tr -d '\000')"; \
...
#language=regexp
define phil_usage_match_target
(^|\n)##\n(#.*\n)+(\.[A-Z_]+:.*\n)*
endef
